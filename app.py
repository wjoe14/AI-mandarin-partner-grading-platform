...
if not SUPABASE_URL or not SUPABASE_ANON_KEY:
    st.error("缺少 SUPABASE_URL 或 SUPABASE_ANON_KEY（請到 Streamlit Secrets 設定）")
    st.stop()
...
def sb_upsert(table, rows, on_conflict=None):
    ...
    r = requests.post(url, headers=headers, params=params, data=json.dumps(rows), timeout=30)
    r.raise_for_status()
    return r.json() if r.text else []

def sb_patch(table, match_params, patch_obj):
    url = f"{REST_BASE}/{table}"
    r = requests.patch(url, headers=HEADERS, params=match_params, data=json.dumps(patch_obj), timeout=30)
    r.raise_for_status()
    return r.json() if r.text else []
...
# ====== Domain logic ======
def get_reviewers():
    return sb_get("reviewers", select="reviewer_id,name,is_active", params={"is_active": "eq.true", "order": "name.asc"})

def get_articles_count():
    rows = sb_get("articles", select="count", params={"select": "count"})
    # Supabase returns list of rows, but count via select isn't reliable here; do a normal fetch minimal
    rows2 = sb_get("articles", select="id", params={"limit": 1})
    # Better: use head request with count exact? (keep simple)
    all_ids = sb_get("articles", select="id")
    return len(all_ids)

def init_reviews_for_article(article_id: str, reviewer_ids):
    # upsert empty review rows if missing
...
def get_next_article(reviewer_id: int):
    # find next not submitted review row, then fetch article
    rows = sb_get("reviews", select="article_id,status", params={"reviewer_id": f"eq.{reviewer_id}", "status": "neq.submitted", "order": "article_id.asc", "limit": 1})
    if not rows:
        return None
...
def export_all_df():
    # fetch all then merge in pandas (simple for small data)
    ...
    # rename columns to your required Chinese output
    out = pd.DataFrame({
        "id": df["id"],
...
if mode == "維護者後台端":
    ...
    reviewers = get_reviewers()
    if not reviewers:
        st.error("目前沒有老師名單。請維護者先在 reviewers 表新增 5 位老師姓名。")
        st.stop()
